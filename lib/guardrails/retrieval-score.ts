...
/** Conceptual meta-questions (mood, emotional language) often embed poorly vs review text. */
export const MIN_CONCEPTUAL_COSINE_FLOOR = 0.22;
...
export function itemQualifiesForEvidence(
  ...
  if (
    question &&
    isConceptualMetaQuestion(question) &&
    isConceptualOnTopic(question, item.content) &&
    (sim >= MIN_CONCEPTUAL_COSINE_FLOOR || kw >= MIN_KEYWORD_TS_RANK)
  ) {
    return true;
  }
